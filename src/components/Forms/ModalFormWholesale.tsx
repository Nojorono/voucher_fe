...
const ModalFormWholesale: React.FC<FormAddWholesaleProps> = ({
  ...
  return (
    <Dialog
      open={open}
      handler={() => {}} // Disable default close handler
      size={'xl'}
      dismiss={{ enabled: false }} // Prevent closing by clicking outside or pressing ESC (if supported by library)
    >
      <DialogHeader>
        {method === 'PUT' ? `Update Data Id : ${IdUpdate[0]}` : 'Tambah Data'}
      </DialogHeader>
      <DialogBody className="max-h-[70vh] overflow-y-auto">
        <form onSubmit={handleSubmit(handleFormSubmit)}>
          ...
          <div className="flex justify-end mt-8 mb-15 gap-2">
            <Button
              variant="text"
              color="red"
              onClick={() => {
