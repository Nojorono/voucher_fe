...
const DataTableApproval = memo(
  ({
    dataPhoto,
    onUpdate,
  }: {
    dataPhoto: photoRetailer[];
    ...
    const updateSelectedData = () => {
      ...
      selectedIds.forEach((id, index) => {
        ...
        if (retailer) {
          ...
          console.error(
            `Retailer with ID ${retailer_name} not found in selectedData`,
          );
          showErrorToast(
            `Retailer with ID ${retailer_name} not found in selectedData`,
          );
    ...
    const groupedData = useMemo(() => {
      return dataPhoto.reduce(
        (acc, retailer) => {
          acc[retailer.retailer_id] = retailer.photos.map(
            (photo) => photo.image,
          );
          return acc;
        },
    ...
    const transformedData = useMemo(() => {
      return Object.entries(groupedData).map(([retailer_id, images]) => {
        const retailer = dataPhoto.find(
          (r) => r.retailer_id === Number(retailer_id),
        );
        const is_verified = retailer?.photos.every((photo) => photo.is_verified)
          ? 1
          : 0;
        const is_approved = retailer?.photos.every((photo) => photo.is_approved)
          ? 1
          : 0;
        const is_rejected = retailer?.photos.every((photo) => photo.is_rejected)
          ? 1
          : 0;

        const statusMap: {
          [key: string]: { status: string; statusColor: string };
        } = {
        ...
        const { status, statusColor } = statusMap[key] || {
          status: 'No Status',
    ...
    const columns: TableColumn<{
      ...
      return [
        ...
        createColumn('Status', (row) => (
          <div
            className={`inline-flex rounded-full bg-opacity-10 py-1 px-2 text-xs ${
              statusClasses[row.status] || 'bg-warning text-warning'
            }`}
          >
            {row.status}
          </div>
        )),
        {
          name: 'Kode Voucher',
          cell: (row) =>
            row.status !== 'Rejected' && row.status !== 'Not Verified' ? (
    ...
    const filteredData = useMemo(() => {
      return transformedData
        .filter((item) => {
          ...
          if (verified === 0)
            return item.is_verified === 0 && item.is_approved === 0;
          if (verified === 2)
            return item.is_verified === 1 && item.is_approved === 0;
    ...
    return (
      <div>
        ...
        <div className="flex justify-end space-x-1 mt-2">
          <button
            className="flex justify-center rounded bg-red-500 p-2 font-medium text-white hover:bg-red-600"
          ...
          <button
            className="flex justify-center rounded bg-green-500 p-2 font-medium text-white hover:bg-green-600"
